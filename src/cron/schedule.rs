...
mod tests {
    ...
    fn no_tz_defaults_to_local_timezone() {
        // Without explicit tz, the schedule should use the OS local timezone.
        ...
        // Both must produce a valid future time
        assert!(next_local > from);
        assert!(next_utc > from);
        // The local-tz result should equal the UTC result only when the OS
        // timezone happens to be UTC; otherwise they differ by the offset.
